...
const fetchGeofencesAtPoint = async (ownerId, latitude, longitude) => {
  if (!ownerId || latitude == null || longitude == null) {
    return [];
  }
  const result = await db.query(
    ...
    `,
    [ownerId, longitude, latitude],
  );

  return result.rows.map((row) => ({
    geofence_id: `geo_${row.id}`,
...
router.post('/location', async (req, res) => {
  ...
  try {
    ...
    const numericId =
      typeof vehicle_id === 'string' && vehicle_id.startsWith('veh_')
...
router.get('/location/:vehicle_id', async (req, res) => {
  try {
    ...
    const numericId =
      v_id.startsWith('veh_') ? parseInt(v_id.split('_')[1], 10) : parseInt(v_id, 10);
    ...
    const locQuery = await db.query(
      'SELECT ST_Y(geom::geometry) AS lat, ST_X(geom::geometry) AS lng, timestamp FROM vehicle_locations WHERE vehicle_id = $1 ORDER BY timestamp DESC LIMIT 1',
      [numericId],
    );
    ...
    const ownerId = vehicleQuery.rows[0].user_id || req.user.id;
    const current_geofences = await fetchGeofencesAtPoint(
      ownerId,
