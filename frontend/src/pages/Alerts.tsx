...
import { Bell, ShieldAlert, Zap, History, ExternalLink, Filter } from 'lucide-react';
...
export const Alerts = () => {
  ...
  const handleConfigureAlert = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await api.alerts.configure(formData);
      fetchAlerts();
    } catch (error) {
    ...
    <div className="motion-safe:animate-in fade-in duration-500 space-y-8">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-outline font-medium mt-2 max-w-xl">Advanced rule-based logic for autonomous fleet monitoring and precision execution.</p>
        </div>
        <div className="flex gap-4">
          <div className="bg-surface-container-low p-4 text-right">
            <p className="text-2xl font-display">{alerts.length}</p>
            <p className="text-[10px] font-bold text-outline uppercase tracking-widest">Active Rules</p>
          </div>
          <div className="bg-surface-container-low p-4 text-right">
            <p className="text-2xl font-display">0.4s</p>
            <p className="text-[10px] font-bold text-outline uppercase tracking-widest">Latency Avg</p>
          </div>
        </div>
      </header>
        ...
        <div className="lg:col-span-4">
          <Card title="Rule Logic Builder" subtitle="Configure new trigger parameters">
            <form className="space-y-6 mt-4" onSubmit={handleConfigureAlert}>
              <div>
                <label className="block text-[10px] font-bold tracking-[0.2em] mb-2 text-outline uppercase">Asset Identifier</label>
                <Input 
              ...
              <div>
                <label className="block text-[10px] font-bold tracking-[0.2em] mb-2 text-outline uppercase">Trigger Condition</label>
                <div className="space-y-2">
                  {[
                    ...
                    >
                      ...
                      <cond.icon className={`w-4 h-4 ${formData.type === cond.id ? 'text-primary' : 'text-outline'}`} />
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div>
                <label className="block text-[10px] font-bold tracking-[0.2em] mb-2 text-outline uppercase">Threshold Value</label>
                <Input 
                  type="number" 
              ...
              <div className="pt-4">
                <Button variant="primary" className="w-full" type="submit">Deploy Rule</Button>
              </div>
            </form>
          </Card>
        </div>
        ...
        <div className="lg:col-span-8 space-y-8">
          <div>
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-sm font-display tracking-[0.2em] uppercase">Active Command Rules</h3>
              ...
              {alerts.map((rule) => (
                ...
                >
                  <div className="flex justify-between items-start">
                    <div>
                      <span className={`text-[10px] font-black uppercase tracking-widest block mb-1 ${rule.type === 'SPEED_LIMIT' ? 'text-primary' : 'text-red-600'}`}>
                        {rule.type}
                      </span>
                      <h4 className="text-xl font-display tracking-tighter uppercase">Threshold: {rule.threshold}</h4>
                    </div>
                  ...
                  <div className="flex items-center gap-4">
                    <span className="text-[10px] font-bold text-outline uppercase tracking-widest">Asset: {rule.vehicle_id}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div>
            <h3 className="text-sm font-display tracking-[0.2em] uppercase mb-4">Execution History</h3>
            <Card className="p-0 overflow-hidden">
              <table className="w-full text-left font-sans text-xs">
                ...
                <tbody className="divide-y divide-outline-variant">
                  {[
                    { time: '14:22:09.11', rule: 'ZONE_EXIT_B', asset: 'DR-092-ALPHA', status: 'SENT', color: 'bg-red-600' },
                    { time: '13:58:44.82', rule: 'SPEED_VIOL_X', asset: 'VT-441-OMEGA', status: 'SUPPRESSED', color: 'bg-outline' },
                    { time: '12:15:33.04', rule: 'FUEL_CRIT_01', asset: 'DR-112-BETA', status: 'EXECUTED', color: 'bg-primary' },
                  ].map((log, i) => (
