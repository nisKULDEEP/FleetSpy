...
import { History, Search, Download, Calendar, ShieldAlert, Zap, Filter } from 'lucide-react';
...
export const Violations = () => {
  ...
  const fetchHistory = async () => {
    try {
      const data = await api.violations.history();
      setViolations(data);
    } catch (error) {
    ...
    return (
      <div className="flex items-center justify-center h-[calc(100vh-10rem)]">
        <div className="text-center">
          ...
          <p className="font-display tracking-widest uppercase text-xs">Auditing Security Logs...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="motion-safe:animate-in fade-in duration-500 space-y-8">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-outline font-medium mt-2 max-w-xl">Audit log of all security breaches, operational violations, and containment failures.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="bg-red-50 border-red-200">
          <div className="flex items-center gap-3 text-red-600 mb-2">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest">Critical Breaches</span>
          </div>
        ...
        <Card>
          <div className="flex items-center gap-3 text-primary mb-2">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest">Speed Violations</span>
          </div>
      ...
      <Card>
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          ...
          <div className="flex gap-2">
            ...
            <Button variant="ghost"><Filter className="w-4 h-4" /></Button>
          </div>
        </div>

        <div className="space-y-4">
          {violations.map((v) => (
            <div key={v.id} className="p-6 bg-surface-container-low border-l-4 border-red-600 flex flex-col md:flex-row md:items-center justify-between gap-4 group hover:bg-surface-container-highest transition-colors">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-1">
                  <span className="text-[10px] font-bold text-red-600 uppercase tracking-widest">Security Breach</span>
                ...
                <div className="flex gap-4 mt-2">
                  <span className="text-[10px] font-bold text-outline uppercase">Asset: {v.vehicle_id}</span>
                  <span className="text-[10px] font-bold text-outline uppercase">Zone: {v.geofence_id}</span>
                </div>
              </div>
              <Button variant="outline" size="sm" className="text-[10px]">View Details</Button>
            </div>
          ))}
        </div>
        ...
        <div className="mt-8 pt-8 border-t border-outline-variant flex justify-between items-center text-[10px] font-bold text-outline uppercase tracking-widest">
          ...
          <div className="flex gap-4">
            <button className="hover:text-on-surface disabled:opacity-30" disabled>Previous</button>
