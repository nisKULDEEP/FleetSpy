...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = 'primary', size = 'md', ...props }, ref) => {
    const variants = {
      ...
      outline:
        'bg-transparent border-2 border-on-surface text-on-surface hover:bg-on-surface hover:text-surface',
...
export const Card = ({
  ...
  return (
    <div
      className={cn(
        'bg-surface p-6 tactical-shadow border-l-4 border-primary-container',
        className,
      )}
    >
      {(title || subtitle) && (
        <div className="mb-6">
          ...
          {subtitle && (
            <p className="text-[10px] font-semibold tracking-widest text-outline uppercase mt-1">
              {subtitle}
            </p>
          )}
        </div>
